...
const startup = async () => {
    try {
        ...
        if (typeof rejectReason === 'object' && rejectReason.message) {
            let errorMessage = rejectReason.message;
            if (rejectReason.message === 'Swagger validation errors:') {
                errorMessage = util.format(`${errorMessage} %j`, rejectReason.validationErrors);
            }
            error.message = util.format("Terminating app, startup due to: %s", errorMessage);
